...
export const SummaryEmployeesExportTableCosts: React.FC<ISummaryEmployeesExportTableProps> = ({statisticForTable, isOpen}) => {
    ...
    useEffect(() => {
        ...
        statUserForExport.reduce((sum: any, cur: any) => {
            setAllSumForTable(prev => prev + cur.all_sum)
        }, 0)
    }, [statUserForExport])
    ...
    const sumTotalForProj: any = (proj: any) => {
        ...
        statisticForTable.forEach((user: any) => {
            ...
            sum = user.data.filter((item: any) => item.project.id === proj.id).reduce((_: any, current: any) => {
                amount = amount + current.expense?.sum
            }, amount)
