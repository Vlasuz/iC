...
export const PopupDeleteEmployee: React.FC<IPopupDeleteEmployeeProps> = (dataItem) => {
    ...
    const handleDelete = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        getBearer('delete')
        axios.delete(getApiLink('/api/admin/employee/delete/?employee_id=' + dataItem.data.id)).then(({data}) => {
            console.log(data)
            if (!data.status) return;

            setIsPopupActive(false)
            dispatch(removeEmployee(dataItem.data))
