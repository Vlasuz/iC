...
export const TableExportCustom: React.FC<ITableExportProps> = ({
    ...
    return (
        <div ref={rootEl}
             className={isExportSelectOpen ? "section-table__export drop-down is-right-default is-active" : "section-table__export drop-down is-right-default"}>
            <button onClick={_ => {
                setIsExportSelectOpen(prev => !prev)
                onClick && onClick()
            }} className="section-table__export--target drop-down__target" type="button">
                {title === 'export all' ? <Translate>summary_page.main.export_all</Translate> :
